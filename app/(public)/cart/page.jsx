...
import {
  ...
  CheckCircle, ShieldCheck, AlertTriangle, X, Gift, Tag,
  Package, Percent, Sparkles, TrendingDown, AlertCircle, RefreshCw,
} from 'lucide-react';
...
export default function CartPage() {
  ...
  const fetchLiveStock = useCallback(async () => {
    ...
    try {
      ...
      let headers = {};
      if (isSignedIn) { const token = await getToken(); headers = { Authorization: `Bearer ${token}` }; }
      const { data } = await axios.get('/api/cart', { headers });
      const map = {};
      (data.items || []).forEach((item) => {
        map[item.variantId || item.id] = item.availableStock ?? item.stock ?? 0;
      });
      setStockMap(map);
    } catch {
      // Silent
  ...
  }, [cartItems.length, isSignedIn, getToken]);
  ...
  const updateQtyHandler = (item, newQty) => {
    ...
    const available = stockMap[key] ?? Infinity;
    if (newQty > available) {
  ...
  const applyCoupon = async () => {
    ...
    if (!isSignedIn) { toast.error('Please login to apply coupon'); return; }
    try {
      const token = await getToken();
      const { data } = await axios.post('/api/coupon', { code: couponCode.trim() }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-slate-100">
          <div className="w-24 h-24 mx-auto mb-6 flex items-center justify-center rounded-full bg-green-50/50"><ShoppingCart size={40} className="text-green-500" /></div>
    ...
    <>
      {showConfirmation && productToRemove && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-5">
              <p className="text-slate-600 mb-4">Remove <span className="font-medium">{productToRemove.name}</span> from cart?</p>
              <div className="flex gap-3">
                <button onClick={() => { setShowConfirmation(false); setProductToRemove(null); }} className="flex-1 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg font-medium hover:bg-slate-50">Cancel</button>
                <button onClick={() => { dispatch(removeFromCart(productToRemove.id)); setShowConfirmation(false); setProductToRemove(null); toast.success('Removed from cart'); }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 mb-1 flex items-center gap-3"><ShoppingCart size={24} className="text-green-500" /> My Cart</h1>
            <p className="text-slate-500">{cartItems.length} {cartItems.length === 1 ? 'item' : 'items'}</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={fetchLiveStock} disabled={stockLoading} className="flex items-center gap-2 text-sm text-slate-500 border border-slate-200 px-3 py-2 rounded-lg hover:bg-slate-50 disabled:opacity-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {cartItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className={`grid grid-cols-1 md:grid-cols-12 gap-4 p-6 items-center ${isOut ? 'bg-red-50/40' : ''}`}>
                      {/* Product */}
                      <div className="col-span-6 flex gap-4 items-center">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <button onClick={() => { dispatch(addToWishlist({ id: item.id, name: item.name, price: item.price, image: item.image, category: item.category, inStock: true })); toast.success('Saved to wishlist', { icon: '❤️' }); }}
                      ...
                      <div className="col-span-2">
                        <div className="flex items-center justify-center">
                          <button onClick={() => updateQtyHandler(item, item.quantity - 1)} disabled={item.quantity <= 1 || isOut}
                            className="p-1.5 rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 disabled:opacity-50">
                            <Minus size={16} />
                          </button>
                          <span className={`w-12 text-center font-medium ${isOut ? 'text-red-500 line-through' : 'text-slate-800'}`}>{item.quantity}</span>
                          <button onClick={() => updateQtyHandler(item, item.quantity + 1)} disabled={isOut || atMax}
                            className="p-1.5 rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 disabled:opacity-50">
                            <Plus size={16} />
                          </button>
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mt-6">
              <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2"><Gift size={18} className="text-purple-500" /> Apply Coupon</h3>
              {appliedCoupon ? (
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Enter coupon code" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyCoupon()}
