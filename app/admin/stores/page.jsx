...
import { BuildingIcon, StoreIcon, AlertCircleIcon, PercentIcon } from 'lucide-react';
  ...
  const saveCommission = async (storeId) => {
    ...
    try {
      ...
      const { data } = await axios.patch(
        '/api/admin/stores',
        { storeId, commissionPercentage: pct },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(data.message);
    ...
    <div className="text-slate-500 mb-28 pt-4 md:p-6">
      ...
      {stores.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-80 bg-slate-50 rounded-xl border border-slate-200">
          ...
          <p className="text-slate-400 mt-2">Approve stores from the Approve page</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-5 mt-6">
          {stores.map((store) => (
            <div
              key={store.id}
              className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 md:p-6 flex max-md:flex-col gap-5 md:items-center max-w-4xl hover:shadow-md transition-shadow"
            >
              <div className="flex-1">
                ...
                <div className="mt-3 flex items-center gap-2">
                  ...
                  {editingCommission === store.id ? (
                    ...
                    <button
                      onClick={() => {
                        setEditingCommission(store.id);
                        setCommissionInput(store.commission?.percentage ?? 0);
                      }}
