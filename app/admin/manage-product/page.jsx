// app/admin/manage-product/page.jsx
// Shows ONLY admin-created products (createdBy = 'ADMIN')
// Edit/Delete/Stock control — mirrors store manage-product but uses /api/products
'use client';
...
function StockControl({ product, onQuantityChange }) {
  ...
  const saveQuantity = useCallback(
    async (newQty) => {
      ...
      try {
        ...
        // Admin uses /api/products PUT to update quantity
        ...
        const newInStock = clamped > 0;
        setQty(clamped);
        onQuantityChange(product.id, clamped, newInStock);
    ...
    [getToken, product, onQuantityChange, editing]
  );
  ...
  const handleInputChange = (e) => {
    ...
    if (val === '' || /^\d+$/.test(val)) setQty(val === '' ? 0 : Number(val));
  };
    ...
    <div className="flex items-center gap-1">
      ...
      {editing ? (
        <input
          ...
          className="w-14 h-7 text-center text-sm font-semibold text-slate-700 border border-green-400 rounded-md outline-none ring-2 ring-green-100 bg-white [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
        />
      ) : (
        <button
          onClick={handleClick}
          className="w-14 h-7 text-center text-sm font-semibold text-slate-700 border border-slate-200 rounded-md hover:border-green-300 hover:bg-green-50 transition-colors cursor-text"
        >
          {saving ? (
            <Loader2 size={12} className="animate-spin mx-auto text-green-400" />
          ) : saved ? (
...
export default function AdminManageProductPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const token = await getToken();
      // GET /api/products — admin sees all, we filter to createdBy=ADMIN client-side
      const { data } = await axios.get('/api/products', {
        headers: { Authorization: `Bearer ${token}` },
      });
      // Show ONLY admin-created products in admin manage page
      const adminProducts = (data.products || []).filter((p) => p.createdBy === 'ADMIN');
  ...
  const confirmDelete = async () => {
    try {
      setDeleting(true);
      const token = await getToken();
      await axios.delete(`/api/products?id=${deleteConfirm.productId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">
              Admin-created global products — visible to all users
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-lg font-medium text-slate-600">No global products yet</p>
              <p className="text-sm mt-1">
                Click "Add Product" to create your first global product
              </p>
              <button
                onClick={() => router.push('/admin/add-product')}
                className="mt-4 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors"
              >
                <PlusCircle size={16} />
                Add First Product
              </button>
