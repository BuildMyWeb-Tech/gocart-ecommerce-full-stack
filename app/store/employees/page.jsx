...
import {
  ...
  Loader2, RefreshCw, UserCheck, UserX, ShieldCheck, ShieldAlert,
} from 'lucide-react';
import { PERMISSIONS } from '@/middlewares/authEmployee';

// ── Permission definitions matching PERMISSIONS keys ──────────────
const ALL_PERMISSIONS = [
  { key: PERMISSIONS.ADD_PRODUCT,           label: 'Add Product',           desc: 'Create new products' },
  { key: PERMISSIONS.EDIT_PRODUCT,          label: 'Edit Product',          desc: 'Update product details and variants' },
  { key: PERMISSIONS.DELETE_PRODUCT,        label: 'Delete Product',        desc: 'Remove products from store' },
  ...
  { key: PERMISSIONS.MANAGE_CATEGORIES,     label: 'Manage Categories',     desc: 'Create and edit store categories' },
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {granted.slice(0, 3).map((p) => (
        <span key={p} className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full capitalize">
          {p.replace(/_/g, ' ').toLowerCase()}
        </span>
      ))}
      {granted.length > 3 && <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-xs rounded-full">+{granted.length - 3} more</span>}
    </div>
  );
}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        const { data } = await axios.post('/api/employee/create', { name: form.name, email: form.email, password: form.password, permissions: form.permissions }, { headers });
  ...
  return (
    <div className="pb-20">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2"><Users size={22} className="text-green-600" /> Employee Management</h1>
              ...
              <tbody className="divide-y divide-slate-100">
                {employees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-4 max-w-[220px]"><PermissionBadge permissions={emp.permissions} /></td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(emp)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-200" title="Edit"><Pencil size={14} /></button>
                        <button onClick={() => setDeleteId(emp.id)} className="p-2 bg-red-50 text-red-500 rounded-lg hover:bg-red-100 border border-red-200" title="Delete"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg my-8">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                  Password {editingEmployee && <span className="text-slate-400 font-normal text-xs">(leave blank to keep)</span>}
                </label>
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••"
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {ALL_PERMISSIONS.map((perm) => (
                    <div key={perm.key} onClick={() => togglePerm(perm.key)}
                      className={`flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer select-none transition-all ${form.permissions[perm.key] ? 'border-green-400 bg-green-50' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100">
              ...
              <button onClick={handleSave} disabled={saving} className="px-5 py-2 text-sm bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium disabled:opacity-60 flex items-center gap-2">
