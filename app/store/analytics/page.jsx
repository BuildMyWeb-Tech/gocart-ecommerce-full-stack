...
// ─────────────────────────────────────────────────────────────────────────────
// STORE SALES REPORT PAGE
// Full analytics dashboard: KPI cards, line/bar chart, pie chart, export
// ─────────────────────────────────────────────────────────────────────────────
'use client';
...
import {
  ...
  BarChart2, Download, RefreshCcw, Calendar, Loader2,
...
const CHART_TABS = [{ value: 'line', label: 'Sales Trend' }, { value: 'bar', label: 'Daily Revenue' }];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <p className="text-xs text-slate-500 font-medium mb-3">Revenue: {labels[0]} vs {labels[1]}</p>
        <div className="flex items-end justify-between">
          <div>
            <p className="text-xs text-slate-400">{labels[0]}</p>
            <p className="text-xl font-bold text-slate-800">₹{revenue.current.toLocaleString('en-IN')}</p>
          </div>
          <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${revenue.growth > 0 ? 'bg-green-100 text-green-700' : revenue.growth < 0 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
            <RevIcon size={12} />{revenue.growth > 0 ? '+' : ''}{revenue.growth}%
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-400">{labels[1]}</p>
            <p className="text-lg font-semibold text-slate-600">₹{revenue.previous.toLocaleString('en-IN')}</p>
          </div>
        </div>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <p className="text-xs text-slate-500 font-medium mb-3">Orders: {labels[0]} vs {labels[1]}</p>
        <div className="flex items-end justify-between">
          <div>
            <p className="text-xs text-slate-400">{labels[0]}</p>
            <p className="text-xl font-bold text-slate-800">{orders.current}</p>
          </div>
          <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${orders.growth > 0 ? 'bg-green-100 text-green-700' : orders.growth < 0 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
            <OrdIcon size={12} />{orders.growth > 0 ? '+' : ''}{orders.growth}%
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-400">{labels[1]}</p>
            <p className="text-lg font-semibold text-slate-600">{orders.previous}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const handleExport = async (format) => {
    try {
      ...
      if (format === 'csv') {
        ...
        const res = await axios.get(`/api/reports/export?${qs}`, {
        ...
        // PDF: fetch data then trigger browser print
        const qs  = buildQS({ format: 'pdf' });
        const res = await axios.get(`/api/reports/export?${qs}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const generatePDF = ({ summary: s, rows, columns }) => {
    ...
    win.document.write(`
      ...
      <style>
        ...
        .kpi-label{font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em}
      ...
      <div class="kpis">
        <div class="kpi"><div class="kpi-label">Total Revenue</div><div class="kpi-value">₹${s.totalRevenue.toLocaleString('en-IN')}</div></div>
        <div class="kpi"><div class="kpi-label">Total Orders</div><div class="kpi-value">${s.totalOrders}</div></div>
        <div class="kpi"><div class="kpi-label">Avg Order Value</div><div class="kpi-value">₹${s.aov.toLocaleString('en-IN')}</div></div>
      </div>
      <table>
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={fetchAll} className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-lg text-slate-700 text-sm transition-all">
          ...
          <div className="relative">
            <button
              ...
              disabled={exporting}
              className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-60"
            >
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {period === 'custom' && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={fetchAll} className="mt-4 bg-green-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-green-700">
                Apply
              </button>
            </div>
          )}
