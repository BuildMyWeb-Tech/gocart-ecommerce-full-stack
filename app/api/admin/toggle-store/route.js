...
// POST /api/admin/toggle-store — Activate or Deactivate an approved store
export async function POST(request) {
  try {
    ...
    // Can only toggle stores that have been approved (ACTIVE or INACTIVE)
    if (store.status === 'PENDING' || store.status === 'REJECTED') {
      return NextResponse.json(
        { error: 'Cannot toggle a store that has not been approved yet' },
        { status: 400 }
      );
    }
