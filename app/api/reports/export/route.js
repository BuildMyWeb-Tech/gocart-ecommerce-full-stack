...
//
// Exports Sales data as CSV or PDF.
// Query params:
//   format  = csv | pdf
...
export async function GET(request) {
  try {
    ...
    const sales = await prisma.sale.findMany({
      ...
      take: 10000, // cap for export safety
    });
    ...
    // ── PDF: return structured JSON for client-side pdf generation ─
    // The frontend uses this data + jsPDF/html2canvas to generate the PDF.
    // We return a clean JSON payload the client can render.
    const summary = {
      totalRevenue: round2(rows.reduce((s, r) => s + r.amount, 0)),
      totalOrders:  rows.length,
      aov:          rows.length > 0 ? round2(rows.reduce((s, r) => s + r.amount, 0) / rows.length) : 0,
      period,
