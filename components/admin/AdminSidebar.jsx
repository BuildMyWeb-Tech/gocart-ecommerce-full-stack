...
// UPDATED: Added "Sales Report" nav link
'use client';
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import {
  LayoutDashboard, Store, CheckSquare, Layers,
  Ticket, PlusCircle, PackageSearch, BarChart2,
} from 'lucide-react';
        ...
        {navLinks.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${isActive ? 'bg-green-50 text-green-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-800'}`}>
