...
'use client';
import { StarIcon, ShoppingCart, Heart, Eye, Zap } from 'lucide-react';
...
const ProductCard = ({ product, badgeText, badgeIcon }) => {
  ...
  const categoryNames   = (product.categories || []).map((c) => c.category?.name || c.name).filter(Boolean);
  const categoryDisplay = categoryNames[0] || (Array.isArray(product.category) ? product.category[0] : product.category) || '';
    ...
    >
      <div className="rounded-2xl overflow-hidden bg-white border border-slate-100 hover:border-green-200 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col h-full">
        ...
        <div className="relative h-52 bg-gradient-to-br from-slate-50 to-slate-100 overflow-hidden">
          ...
          {(product.images || []).map((img, i) => (
            <Image key={i} src={img} alt={product.name} fill
              className={`object-contain p-4 transition-all duration-500 ${i === activeImageIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
          ...
          <button
            ...
            className={`absolute top-3 right-3 z-20 p-2 rounded-full shadow-md transition-all duration-200 ${isInWishlist ? 'bg-red-500 text-white scale-110' : 'bg-white/90 text-slate-400 hover:text-red-500 hover:bg-white'}`}
          >
          ...
          {/* Badge */}
          {badgeText && (
            <div className="absolute top-3 left-3 z-20 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 shadow">
              {badgeIcon} {badgeText}
            </div>
          )}
          ...
          {/* Category pill */}
          {categoryDisplay && (
            <div className="absolute bottom-3 left-3 z-20 bg-white/90 backdrop-blur-sm text-[10px] text-slate-600 font-medium px-2 py-0.5 rounded-full border border-slate-200/60">
              {categoryDisplay}
            </div>
          )}
          ...
          {/* Quick view on hover */}
          <div className="absolute bottom-3 right-3 z-20 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-1 group-hover:translate-y-0">
        ...
        <div className="p-4 flex flex-col flex-1">
          {product.brand && (
            <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide mb-1">{product.brand}</p>
          )}

          <h3 className="text-sm font-semibold text-slate-800 line-clamp-2 leading-snug mb-2 group-hover:text-green-700 transition-colors min-h-[2.5rem]">
            {product.name}
          </h3>
          ...
          <div className="flex items-center gap-1.5 mb-3">
            <div className="flex">
              {Array(5).fill('').map((_, i) => (
                <StarIcon key={i} size={12} fill={avgRating >= i + 1 ? '#f59e0b' : '#e2e8f0'} strokeWidth={0} />
              ))}
            </div>
            <span className="text-[10px] text-slate-500">{avgRating.toFixed(1)} ({ratings.length})</span>
          </div>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {isInStock ? (
              ...
              <span className="text-[10px] text-red-500 font-semibold bg-red-50 px-2 py-0.5 rounded-full">Sold Out</span>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors border border-slate-200"
            >
              <Eye size={14} /> Details
            </button>
            <button
