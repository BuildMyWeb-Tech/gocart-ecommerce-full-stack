...
// UPDATED: "Sales Report" href now points to /store/analytics (already in your sidebar)
// No structural change — just confirming the link is correct.
// Your existing StoreSidebar already has:
//   { name: 'Sales Report', href: '/store/analytics', icon: BarChart2 }
// ✅ No change needed to StoreSidebar — the link is already there.
// Just make sure the page file exists at:
//   app/store/analytics/page.jsx   ← the file you received above

// If you want to copy the full updated sidebar, here it is:
'use client';
import { usePathname } from 'next/navigation';
import {
  HomeIcon, LayoutListIcon, SquarePenIcon, SquarePlusIcon,
  BarChart2, ShoppingBag, Settings, HelpCircle, X, ChevronRight, ChevronLeft,
} from 'lucide-react';
  ...
  return (
    <div className={`inline-flex h-full flex-col gap-3 border-r border-slate-200 bg-white shadow-sm transition-all relative ${collapsed ? 'sm:w-20' : 'sm:min-w-64'} w-72`}>
      ...
      <div className={`flex ${collapsed ? 'flex-col' : 'flex-row'} gap-3 items-center pt-6 px-4 pb-2`}>
        ...
        <button onClick={() => setCollapsed(!collapsed)} className="text-slate-400 hover:text-slate-600 p-1 rounded-md hover:bg-slate-100 hidden md:block">
        ...
        {sidebarLinks.map((link) => {
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-3 p-2.5 ${collapsed ? 'justify-center' : 'pl-6 pr-3'} transition-all hover:bg-slate-50 ${isActive ? 'bg-gradient-to-r from-slate-50 to-slate-100 font-medium text-slate-800' : 'text-slate-500 hover:text-slate-600'}`}
            >
